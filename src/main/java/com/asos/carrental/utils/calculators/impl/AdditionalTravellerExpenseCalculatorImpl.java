...
public class AdditionalTravellerExpenseCalculatorImpl
	...
	@Override
	public BigDecimal calculateAdditionalExpense(Vehicle vehicle,
		...
		if (travellerCount > maxPassengerCapacity) {
			int additionalTravellerCount = travellerCount
					- maxPassengerCapacity;
			BigDecimal additionalTravellerRate = rateRepository
					.findAdditionalTravellerRate();
			return additionalTravellerRate.multiply(new BigDecimal(distance))
					.multiply(new BigDecimal(additionalTravellerCount))
